...
def harmonize_MR(df_exposure, df_outcome, action=2, eaf_threshold=0.42):
    ...
    if action == 3: #Simply delete them
        ...
        diff_row=nrow-df.shape[0]
        print(f"Action = 3: excluding {diff_row} palindromic SNPs: {snps_deleted}")
        ...
        df = df[~df.ambiguous]
        diff = nrow - df.shape[0]
        if diff > 0: print(f"Action = 2: {diff} SNPs excluded for being palindromic with intermediate allele frequencies.")
