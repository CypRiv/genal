...
def fill_ea_nea(data, reference_panel_df):
    ...
    print(
        f"Alleles columns created: effect (EA) and non-effect allele (NEA). {n_missing}({n_missing/data.shape[0]*100:.3f}%) values are set to nan because SNPs were not found in the reference data."
    )
    return data
...
def fill_nea(data, reference_panel_df):
    ...
    print(
        f"The NEA (Non Effect Allele) column has been created. {n_missing}({n_missing/data.shape[0]*100:.3f}%) values are set to nan because SNPs were not found in the reference data."
    )
    return data
...
def fill_snpids_func(data, reference_panel_df):
    ...
    print(
        f"The SNP column (rsID) has been created. {n_missing}({n_missing/data.shape[0]*100:.3f}%) SNPs were not found in the reference data{print_statement if standard_name_condition else ''}."
    )
    return data
